...
export const CategoryCard = ({ imageAlt, imageSrc, linkUrl, name, title }: CategoryCardProps) => {
  return (
    <FeatureSection name={name} asChild>
      <article className='rounded-xl transition-all duration-300 ease-in-out hover:-translate-y-3 hover:translate-x-5'>
        <FeatureSectionContent className='gap-8 px-8 py-10 transition-all duration-300 ease-in-out hover:rounded-xl hover:shadow-strong tablet:grid-cols-[300px_1fr] lg:gap-2 lg:py-6 xl:grid-cols-[320px_1fr]'>
          ...
          <FeatureSectionImage className='h-80 w-full tablet:order-3 lg:h-[285px] lg:justify-end xl:h-[270px] super_desktop:h-64'>
            <Image
              className='object-contain lg:translate-x-[56px] lg:translate-y-6 lg:scale-[1.4] min-[1400px]:translate-y-4'
